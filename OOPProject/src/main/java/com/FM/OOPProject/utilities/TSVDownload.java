...
public class TSVDownload {
	...
	public static void GetTSV() throws JSONException, MalformedURLException, IOException {
		if (!Files.exists(Paths.get("t.tsv"))) {
			JSONObject obj = new JSONObject(
					readurl("http://data.europa.eu/euodp/data/api/3/action/package_show?id=ojAmzVahjBnws2njEN0qhQ"));
			JSONArray arr = obj.getJSONObject("result").getJSONArray("resources");
			String html = new String();
			for (int i = 0; i < arr.length(); i++) {
				if (arr.getJSONObject(i).getString("format")
						.equals("http://publications.europa.eu/resource/authority/file-type/TSV")) {
	...
	/**
	 * Metodo che effettua il download di un file da url e lo salva in filename
	 * 
	...
	/**
	 * Metodo che legge un file mediante URL e lo ritorna come stringa
	 * 
	 * @param link
